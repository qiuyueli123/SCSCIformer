...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.w_dec_dec = torch.nn.Parameter(torch.FloatTensor([configs.w_lin_dec]*configs.enc_in),requires_grad=True)
        self.w_dec_convd = torch.nn.Parameter(torch.FloatTensor([configs.w_lin_convd]*configs.enc_in),requires_grad=True)
        self.w_dec_orgin = torch.nn.Parameter(torch.FloatTensor([configs.w_lin_orgin]*configs.enc_in),requires_grad=True)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, batch_y,
        ...
        dec_out = self.revin_layer0(self.w_dec_dec*dec_out[:, -self.pred_len:, :] + self.revin_coef*self.w_dec*revin_linear_out + self.convd_coef*self.w_dec_convd*convd_linear_out, 'denorm') + self.orgin_coef*self.w_dec_orgin*orgin_linear_out
